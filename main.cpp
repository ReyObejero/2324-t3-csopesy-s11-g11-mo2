...
int main() {
    ...
    PagingManager* firstfit = new PagingManager(64);

    std::cout << firstfit->maxFrames << " = Max Frames" << std::endl;
    Process* p1 = new Process("P0", 100);
    p1->frames_to_allocate = 32 / 2;

    Process* p2 = new Process("P1", 100);
    p2->frames_to_allocate = 32 / 2;
    Process* p3 = new Process("P2", 100);
    p3->frames_to_allocate = 32 / 2;
    Process* p4 = new Process("P3", 100);
    p4->frames_to_allocate = 32 / 2;
    Process* p5 = new Process("P4", 100);
    p5->frames_to_allocate = 32 / 2;

    int ret = firstfit->allocateMemory(p1);
    std::cout << firstfit->currentFrames << " = Current Remaining Frames, " << ret << " = Return Alloc" << std::endl;
    ret = firstfit->allocateMemory(p2);
    std::cout << firstfit->currentFrames << " = Current Remaining Frames, " << ret << " = Return Alloc" << std::endl;
    ret = firstfit->allocateMemory(p3);
    std::cout << firstfit->currentFrames << " = Current Remaining Frames, " << ret << " = Return Alloc" << std::endl;
    ret = firstfit->allocateMemory(p4);
    std::cout << firstfit->currentFrames << " = Current Remaining Frames, " << ret << " = Return Alloc" << std::endl;
    firstfit->deallocateMemory(p2);
    ret = firstfit->allocateMemory(p5);
    std::cout << firstfit->currentFrames << " = Current Remaining Frames, " << ret << " = Return Alloc" << std::endl;

    std::cout << p2->frames_to_allocate << " = Current ToDO Allocate Frames" << std::endl;
...
//                std::cout << "Memory Usage: 16384MiB / 16384MiB" << std::endl;
//                std::cout << "Memory Util: 100%" << std::endl;
//                std::cout << std::endl;
//                std::cout << "===================================" << std::endl;
//                std::cout << "Running processes and memory usage:" << std::endl;
//                std::cout << "process05 134 MiB" << std::endl;
//                std::cout << "process06 134 MiB" << std::endl;
//                std::cout << "process07 977 MiB" << std::endl;
//                std::cout << "-----------------------------------" << std::endl;
//            }
//
